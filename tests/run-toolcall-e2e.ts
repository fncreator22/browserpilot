...
async function runEndToEndToolCallTest() {
  console.log("=================================================");
  console.log("  BROWSERPILOT END-TO-END TOOL CALLING TEST      ");
  console.log("=================================================\n");

  const jobId = `e2e-toolcall-${Date.now()}`;
  ...
  // 1. Host local test fixture server
  ...
  if (hasLiveApiKey) {
    ...
    console.log("[Notice] No live GEMINI_API_KEY set. Simulating planner output conforming to ActionPlanSchema...");
    // 2c. Synthetic structured plan matching exact Gemini schema format
    actionPlan = {
      goal: userPrompt,
      ...
      steps: [
        {
          stepNumber: 1,
          rationale: "Navigate to the target fixture URL",
          isOptional: false,
        ...
        {
          stepNumber: 2,
          rationale: "Dismiss the promo modal overlay",
          isOptional: false,
          checkpointScreenshot: false,
          action: {
            tool: "browser.click",
...
runEndToEndToolCallTest().catch((err) => {
